...
モデルにツールを必ず使用させるタイミング・方法の制御（`tool_choice`, `toolUseBehavior` など）については [エージェント](/ja/guides/agents#forcing-tool-use) を参照してください。
...
- [ツール使用の強制](/ja/guides/agents#forcing-tool-use) について学ぶ
- [ガードレール](/ja/guides/guardrails) を追加してツールの入力・出力を検証
- [`tool()`](/openai/agents/functions/tool) と各種組み込みツール型の TypeDoc リファレンスを確認
