...
| `model`         | no       | モデル名 **または** カスタム [`Model`](/openai/agents/interfaces/model/) 実装                   |
| `modelSettings` | no       | チューニングパラメーター (temperature, top_p など)                                              |
| `tools`         | no       | モデルが呼び出せる [`Tool`](/openai/agents/type-aliases/tool/) インスタンスの配列               |
...
このパターンの詳細は [ハンドオフ](/ja/guides/handoffs) を参照してください。
...
ガードレールを使用すると、ユーザー入力やエージェント出力の検証・変換が可能です。`inputGuardrails` と `outputGuardrails` 配列で設定します。詳細は [ガードレール](/ja/guides/guardrails) をご覧ください。
...
- [エージェントの実行](/ja/guides/running-agents) を学ぶ
- [ツール](/ja/guides/tools)、[ガードレール](/ja/guides/guardrails)、[モデル](/ja/guides/models) をさらに深掘り
