...
WebRTC の代わりに WebSocket 接続を使うには、セッション作成時に `transport: 'websocket'` または `OpenAIRealtimeWebSocket` インスタンスを渡します。これはサーバーサイドのユースケースに適しており、たとえば Twilio で電話エージェントを構築する場合に有用です。
...
別の speech-to-speech API を使いたい場合や独自のトランスポート機構がある場合は、`RealtimeTransportLayer` インターフェースを実装し、`RealtimeTransportEventTypes` イベントを発行して、自作できます。

## Realtime API とのより直接的な対話

OpenAI Realtime API を使いつつ、Realtime API により直接アクセスしたい場合は、次の 2 つの選択肢があります。

### オプション 1 - トランスポート層へのアクセス

`RealtimeSession` の機能をすべて活用したい場合は、`session.transport` を通じてトランスポート層にアクセスできます。
...
自動ツール実行やガードレールなどが不要な場合は、接続と割り込みのみを管理する「薄い (thin) クライアント」としてトランスポート層だけを利用することもできます。
