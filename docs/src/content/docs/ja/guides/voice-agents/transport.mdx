...
別の speech-to-speech API を使いたい場合や、独自のトランスポート機構がある場合は、`RealtimeTransportLayer` インターフェースを実装し、`RealtimeTransportEventTypes` イベントを発行することで独自に作成できます。
...
自動ツール実行やガードレールなどが不要であれば、接続と割り込みの管理だけを行う "thin" クライアントとしてトランスポート層を使用することもできます。
