...
ハンドオフは、あるエージェントが会話の一部を別のエージェントに委譲する機能です。これは、異なるエージェントが特定の分野を専門としている場合に有用です。たとえばカスタマーサポートアプリでは、予約、返金、FAQ を担当するエージェントを用意できます。
...
プロンプト内でハンドオフに言及すると、LLM の応答がより安定します。SDK は `RECOMMENDED_PROMPT_PREFIX` 経由で推奨プレフィックスを提供します。
