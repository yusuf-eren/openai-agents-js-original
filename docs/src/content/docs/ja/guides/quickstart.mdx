...
1. プロジェクトを作成して npm を初期化します。これは一度だけ実施すれば十分です
...
`run` メソッドでエージェントを実行できます。開始したいエージェントと渡したい入力の両方を指定して実行を開始します。

これにより、その実行中に行われた最終出力とすべてのアクションを含む 実行結果 が返されます。
...
追加のエージェントを同様に定義して、問題を小さな部分に分割し、エージェントが目の前のタスクにより集中できるようにします。また、エージェントごとにモデルを定義することで、問題ごとに異なるモデルを使うこともできます。
...
複数のエージェントをオーケストレーションするために、エージェントに `handoffs` を定義できます。これにより、実行の過程で次のエージェントへ会話を自動的に引き継げるようになります。
...
実行後は、実行結果 の `finalAgent` プロパティを見ることで、どのエージェントが最終応答を生成したかがわかります。
