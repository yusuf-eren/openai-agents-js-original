...
エージェントを実行すると、最終出力と実行の完全な履歴を含む [エージェントの実行結果](/ja/guides/results) オブジェクトが返されます。
...
3. `maxTurns` に達すると [`MaxTurnsExceededError`](/openai/agents-core/classes/maxturnsexceedederror) をスロー
...
入力は文字列、[入力アイテム](/openai/agents-core/type-aliases/agentinputitem) のリスト、または [Human in the loop](/ja/guides/human-in-the-loop) エージェントを構築する場合の [`RunState`](/openai/agents-core/classes/runstate) オブジェクトのいずれかです。
...
| `context`  | –       | すべてのツール / ガードレール / ハンドオフに転送されるコンテキスト オブジェクト。詳しくは [コンテキスト管理](/ja/guides/context) |
| `maxTurns` | `10`    | セーフティ制限 — 到達時に [`MaxTurnsExceededError`](/openai/agents-core/classes/maxturnsexceedederror) をスロー                  |
...
- [`MaxTurnsExceededError`](/openai/agents-core/classes/maxturnsexceedederror) — `maxTurns` に到達
- [`ModelBehaviorError`](/openai/agents-core/classes/modelbehaviorerror) — モデルが無効な出力を生成（例: JSON 形式不正、未知のツール）
- [`InputGuardrailTripwireTriggered`](/openai/agents-core/classes/inputguardrailtripwiretriggered) / [`OutputGuardrailTripwireTriggered`](/openai/agents-core/classes/outputguardrailtripwiretriggered) — ガードレール違反
- [`GuardrailExecutionError`](/openai/agents-core/classes/guardrailexecutionerror) — ガードレールの実行に失敗
- [`ToolCallError`](/openai/agents-core/classes/toolcallerror) — 関数ツール呼び出しが失敗
- [`UserError`](/openai/agents-core/classes/usererror) — 設定やユーザー入力に基づくエラー
...
- [モデル](/ja/guides/models) の設定方法を学ぶ
- エージェントに [ツール](/ja/guides/tools) を提供する
- 本番環境に向けて [ガードレール](/ja/guides/guardrails) や [トレーシング](/ja/guides/tracing) を追加する
