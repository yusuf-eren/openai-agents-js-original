...
音声エージェントは OpenAI の音声対音声モデルを使用して、リアルタイムの音声チャットを提供します。これらのモデルは音声、テキスト、ツール呼び出しのストリーミングに対応し、音声/電話によるカスタマーサポート、モバイルアプリでの体験、音声チャットなどの用途に最適です。
...
音声対音声モデルを使用することで、モデルがリアルタイムに音声を処理できるため、モデルの応答後にテキストへ文字起こしして再度音声に変換し直す必要がありません。
