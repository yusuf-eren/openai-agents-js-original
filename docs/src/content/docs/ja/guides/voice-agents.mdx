...
音声エージェントは OpenAI の speech-to-speech モデルを使って、リアルタイムの音声チャットを提供します。これらのモデルは音声、テキスト、ツール呼び出しのストリーミングに対応し、音声や電話のカスタマーサポート、モバイルアプリの体験、音声チャットなどの用途に最適です。
...
- 音声および割り込みの処理
- ハンドオフによるマルチエージェント オーケストレーション
- ツール定義と呼び出し
- モデル出力を監視するカスタム ガードレール
- ストリーミング イベントのコールバック
- 同じコンポーネントをテキスト エージェントと音声エージェントの両方で再利用

speech-to-speech モデルを使用することで、モデルが動作した後にテキストを音声へ再変換したり、書き起こしを行う必要なく、音声をリアルタイムに処理する能力を活用できます。
