...
音声エージェントは OpenAI の音声対音声モデルを使用して、リアルタイムの音声チャットを提供します。これらのモデルは、音声、テキスト、およびツール呼び出しのストリーミングに対応し、音声/電話でのカスタマーサポート、モバイルアプリでの体験、音声チャットといった用途に最適です。
...
- 音声と割り込みの処理
- ハンドオフを介したマルチエージェントのオーケストレーション
- ツールの定義と呼び出し
- モデル出力を監視するカスタムガードレール
- ストリーミングイベント向けコールバック
- テキストと音声エージェントの両方で同じコンポーネントを再利用

音声対音声モデルを使用することで、モデルがリアルタイムに音声を処理する能力を活用でき、モデルが応答した後に文字起こしを行いテキストを音声へ再変換する必要がありません。
