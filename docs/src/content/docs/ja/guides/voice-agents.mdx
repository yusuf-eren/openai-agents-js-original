...
Voice Agents は OpenAI の speech‑to‑speech モデルを使用して、リアルタイムの音声チャットを提供します。これらのモデルは音声、テキスト、ツール呼び出しのストリーミングに対応し、音声/電話のカスタマーサポート、モバイルアプリ体験、音声チャットなどの用途に最適です。
...
speech‑to‑speech モデルを使用することで、モデルが動作した後にテキストへ文字起こしして再度音声に変換する必要なく、モデルのリアルタイム音声処理能力を活用できます。
