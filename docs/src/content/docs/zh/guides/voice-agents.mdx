...
语音智能体使用 OpenAI 的语音到语音模型提供实时语音聊天。这些模型支持音频、文本与工具调用的流式传输，非常适合语音/电话客服、移动应用体验和语音聊天等场景。

Voice Agents SDK 为 [OpenAI Realtime API](https://platform.openai.com/docs/guides/realtime) 提供了一个 TypeScript 客户端。
...
通过使用语音到语音模型，我们可以利用模型的实时音频处理能力，而无需在模型响应后进行转写再转换回音频。
