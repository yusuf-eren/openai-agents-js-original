...
这将返回一次运行的结果，其中包含最终输出以及期间执行的任何操作。
...
为了在多个智能体之间进行编排，您可以为智能体定义 `handoffs`。这将使智能体能够将对话传递给下一个智能体。这将在一次运行过程中自动发生。
...
运行结束后，您可以通过查看结果上的 `finalAgent` 属性，了解哪个智能体生成了最终响应。

## 运行智能体编排

Runner（运行器）负责处理各个智能体的执行、任何可能的交接以及工具执行。
...
Agents SDK 将自动为您生成追踪。这使您可以审查智能体的运行方式、调用了哪些工具或把流程交接给了哪个智能体。
