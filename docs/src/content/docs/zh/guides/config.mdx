...
默认情况下，SDK 在首次导入时读取环境变量 `OPENAI_API_KEY`。如果无法设置该变量，你可以手动调用 `setDefaultOpenAIKey()`。
...
你也可以传入你自己的 `OpenAI` 客户端实例。否则，SDK 将使用默认密钥自动创建一个。
...
SDK 使用 [`debug`](https://www.npmjs.com/package/debug) 包进行调试日志记录。将环境变量 `DEBUG` 设为 `openai-agents*` 可查看详细日志。

```bash
export DEBUG=openai-agents*
```

你可以使用来自 `@openai/agents` 的 `getLogger(namespace)` 为你的模块获取带命名空间的日志记录器。
