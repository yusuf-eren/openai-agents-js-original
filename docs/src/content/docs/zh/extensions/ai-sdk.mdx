...
<Aside type="caution">
  此适配器仍处于测试阶段。您在使用一些模型提供方（尤其是较小的提供方）时可能会遇到问题。请通过
  [GitHub issues](https://github.com/openai/openai-agents-js/issues)
  反馈问题，我们会尽快修复。
</Aside>

开箱即用，Agents SDK 可通过 Responses API 或 Chat Completions API 使用 OpenAI 模型。如果您希望使用其他模型，[Vercel 的 AI SDK](https://sdk.vercel.ai/) 提供了丰富的受支持模型，可通过此适配器接入 Agents SDK。
...
<Aside type="caution">
  我们当前支持 ai-sdk 的模型提供方 v2 模块，它们与 Vercel AI SDK v5
