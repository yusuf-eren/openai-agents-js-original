...
가드레일은 에이전트와 _병렬로_ 실행되어 사용자 입력 또는 에이전트 출력에 대한 점검과 검증을 수행할 수 있게 합니다. 예를 들어, 비용이 큰 모델을 호출하기 전에 가벼운 모델을 가드레일로 실행할 수 있습니다. 가드레일이 악의적인 사용을 감지하면 오류를 트리거하여 비용이 큰 모델의 실행을 중단할 수 있습니다.
...
> **참고**
> 입력 가드레일은 사용자 입력을 대상으로 하므로, 해당 에이전트가 워크플로에서 _첫 번째_ 에이전트인 경우에만 실행됩니다. 가드레일은 에이전트 자체에 설정합니다. 에이전트별로 요구되는 가드레일이 다를 수 있기 때문입니다.
...
> **참고**
> 출력 가드레일은 해당 에이전트가 워크플로에서 _마지막_ 에이전트인 경우에만 실행됩니다. 실시간 음성 상호작용의 경우 [음성 에이전트 구축](/openai-agents-js/ko/guides/voice-agents/build#guardrails)을 참조하세요.
...
2. 가드레일 함수는 에이전트의 입력 또는 출력을 받아 결과를 반환합니다.
