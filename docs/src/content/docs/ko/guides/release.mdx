...
이 프로젝트는 `0.Y.Z` 형태의 약간 수정된 의미적 버전 관리(semantic versioning)를 따릅니다. 앞의 `0`은 SDK가 아직 빠르게 발전 중임을 의미합니다. 구성 요소 증분 규칙은 다음과 같습니다.

## 마이너(`Y`) 버전

베타로 표시되지 않은 공개 인터페이스에 대한 **호환성 파괴 변경**이 있을 때 마이너 버전 `Y`를 증가시킵니다. 예를 들어 `0.0.x`에서 `0.1.x`로 올라가면 파괴적 변경이 포함될 수 있습니다.

파괴적 변경을 원하지 않는 경우, 프로젝트에서 `0.0.x` 버전에 고정할 것을 권장합니다.
...
무엇이 변경되었는지 이해할 수 있도록 각 서브 패키지에 대해 변경 로그를 생성합니다. 변경이 특정 서브 패키지에서 발생했을 수 있으므로, 해당 변경의 세부 사항은 그 서브 패키지의 변경 로그에서 확인해야 할 수 있습니다.
