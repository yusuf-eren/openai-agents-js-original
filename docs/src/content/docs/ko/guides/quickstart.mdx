...
3. OpenAI API 키를 설정하세요. 없다면 OpenAI API 키를 만드는 방법은 [다음 안내](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)를 참고하세요.
...
`run` 메서드를 사용해 에이전트를 실행할 수 있습니다. 시작하려는 에이전트와 전달할 입력을 함께 넘기면 실행이 트리거됩니다.

이렇게 하면 해당 실행 동안 수행된 모든 액션과 최종 출력을 포함하는 결과가 반환됩니다.
...
추가 에이전트를 유사하게 정의해 문제를 더 작은 부분으로 나누고, 현재 작업에 더 집중하도록 할 수 있습니다. 또한 에이전트에 모델을 정의해 문제 유형마다 다른 모델을 사용할 수 있습니다.
...
멀티 에이전트 오케스트레이션을 위해 에이전트에 `handoffs`를 정의할 수 있습니다. 이렇게 하면 실행 도중 에이전트가 대화를 다음 에이전트로 자동으로 넘길 수 있습니다.
...
실행이 끝나면 결과의 `finalAgent` 속성을 확인해 최종 응답을 생성한 에이전트를 확인할 수 있습니다.

## 에이전트 오케스트레이션 실행

Runner는 개별 에이전트 실행, 잠재적 핸드오프, 도구 실행을 처리합니다.
...
Agents SDK는 자동으로 트레이스를 생성합니다. 이를 통해 에이전트가 어떻게 작동하는지, 어떤 도구를 호출했는지, 어느 에이전트로 핸드오프했는지 검토할 수 있습니다.

에이전트 실행에서 무슨 일이 있었는지 확인하려면
