...
기본적으로 Agents SDK는 Responses API 또는 Chat Completions API를 통해 OpenAI 모델과 함께 작동합니다. 그러나 다른 모델을 사용하려면, 이 어댑터를 통해 [Vercel's AI SDK](https://sdk.vercel.ai/)가 지원하는 다양한 모델을 Agents SDK에 연결할 수 있습니다.
...
<Aside type="caution">
  현재 Vercel AI SDK v5와 호환되는 ai-sdk의 모델 프로바이더 v2 모듈을
  지원합니다. 특별한 이유로 v1 모델 프로바이더를 계속 사용해야 하는 경우
