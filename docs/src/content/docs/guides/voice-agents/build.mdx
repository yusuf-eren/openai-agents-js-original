...
You can configure your session by passing additional options to either the [`RealtimeSession`](/openai/agents-realtime/classes/realtimesession/) during construction or
...
For parameters that are new and don't have a matching parameter in the [RealtimeSessionConfig](/openai/agents-realtime/type-aliases/realtimesessionconfig/) you can use `providerData`. Anything passed in `providerData` will be passed directly as part of the `session` object.
