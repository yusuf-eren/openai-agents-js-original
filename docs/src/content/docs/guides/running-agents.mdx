...
After running your agent, you will receive a [result](/guides/results) object that contains the final output and the full history of the run.
...
3. Throw [`MaxTurnsExceededError`](/openai/agents-core/classes/maxturnsexceedederror) once `maxTurns` is reached.
...
The input can either be a string (which is considered a user message), or a list of [input items](/openai/agents-core/type-aliases/agentinputitem), or a [`RunState`](/openai/agents-core/classes/runstate) object in case you are building a [human-in-the-loop](/guides/human-in-the-loop) agent.
...
| `context`  | –       | Context object forwarded to every tool / guardrail / handoff. Learn more in the [context guide](/guides/context). |
| `maxTurns` | `10`    | Safety limit – throws [`MaxTurnsExceededError`](/openai/agents-core/classes/maxturnsexceedederror) when reached.  |
...
- [`MaxTurnsExceededError`](/openai/agents-core/classes/maxturnsexceedederror) – `maxTurns` reached.
- [`ModelBehaviorError`](/openai/agents-core/classes/modelbehaviorerror) – model produced invalid output (e.g. malformed JSON, unknown tool).
- [`InputGuardrailTripwireTriggered`](/openai/agents-core/classes/inputguardrailtripwiretriggered) / [`OutputGuardrailTripwireTriggered`](/openai/agents-core/classes/outputguardrailtripwiretriggered) – guardrail violations.
- [`GuardrailExecutionError`](/openai/agents-core/classes/guardrailexecutionerror) – guardrails failed to complete.
- [`ToolCallError`](/openai/agents-core/classes/toolcallerror) – any of function tool calls failed.
- [`UserError`](/openai/agents-core/classes/usererror) – any error thrown based on configuration or user input.
...
- Learn how to [configure models](/guides/models).
- Provide your agents with [tools](/guides/tools).
- Add [guardrails](/guides/guardrails) or [tracing](/guides/tracing) for production readiness.
