...
1. **Hosted tools** – run alongside the model on OpenAI servers. _(web search, file search, computer use)_
...
Refer to the [Agents guide](/guides/agents#forcing-tool-use) for controlling when and how a model
...
- Learn about [forcing tool use](/guides/agents#forcing-tool-use).
- Add [guardrails](/guides/guardrails) to validate tool inputs or outputs.
- Dive into the TypeDoc reference for [`tool()`](/openai/agents/functions/tool) and the various hosted tool types.
